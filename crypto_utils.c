...
{
    // Escribe k en big-endian sobre 8 bytes.
    for (int i = 0; i < 8; ++i) {
...
static unsigned char *pkcs7_unpad(unsigned char *buf, size_t *len)
{
    ...
    if (pad == 0 || pad > 8) return NULL; // bloque DES=8
                     ...
                     uint64_t key, unsigned char **out, size_t *out_len)
{
    *out = NULL; *out_len = 0;
    ...
    DES_cblock kblock; key_from_u64(key, &kblock);
    DES_key_schedule sched;
    // OpenSSL 3: DES_set_key_unchecked retorna void (deprecado). Llamamos sin comparar.
    DES_set_key_unchecked(&kblock, &sched);

    unsigned char *cipher = (unsigned char *)malloc(padded_len);
    if (!cipher) { free(padded); return false; }

    for (size_t i = 0; i < padded_len; i += 8) {
        DES_ecb_encrypt((const_DES_cblock *)(padded + i), (DES_cblock *)(cipher + i), &sched, DES_ENCRYPT);
    }

    free(padded);
    *out = cipher; *out_len = padded_len;
    return true;
}
                     ...
                     uint64_t key, unsigned char **out, size_t *out_len)
{
    *out = NULL; *out_len = 0;
    ...
    DES_cblock kblock; key_from_u64(key, &kblock);
    DES_key_schedule sched;
    // OpenSSL 3: retorna void. Llamamos sin comparar.
    DES_set_key_unchecked(&kblock, &sched);
    ...
    for (size_t i = 0; i < clen; i += 8) {
        DES_ecb_encrypt((const_DES_cblock *)(cipher + i), (DES_cblock *)(plain + i), &sched, DES_DECRYPT);
    }

    size_t plen = clen;
    if (!pkcs7_unpad(plain, &plen)) { free(plain); return false; }

    *out = plain; *out_len = plen;
    return true;
}
...
bool buffer_contains_substring(const unsigned char *buf, size_t len, const char *needle)
{
    ...
    // Búsqueda ingenua (se puede optimizar a KMP si se desea)
...
bool load_file(const char *path, unsigned char **buf, size_t *len)
{
    *buf = NULL; *len = 0;
    ...
    fclose(f);
    if (r != (size_t)sz) { free(tmp); return false; }
    *buf = tmp; *len = (size_t)sz; return true;
}
